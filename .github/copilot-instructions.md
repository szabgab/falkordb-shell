...
- For `.tutorial` changes, preserve the current interactive tutorial design unless the task explicitly changes it:
  ...
  - blank ENTER executes the current step, and the following blank ENTER reveals the next step
  - non-empty input should execute as a normal shell command, show only its result, and wait for blank ENTER before revealing the pending tutorial step
